...
export class ApplicationFormComponent {
  ...
  protected readonly fields = form(this.formModel, (fields) => {
    ...
    maxLength(fields.companyName, 250, { message: 'Company name must be 250 characters or fewer.' });
    required(fields.role, { message: 'Role is required.' });
    maxLength(fields.role, 250, { message: 'Role must be 250 characters or fewer.' });
    maxLength(fields.contactName, 250, { message: 'Contact name must be 250 characters or fewer.' });
    maxLength(fields.contactEmail, 250, { message: 'Contact email must be 250 characters or fewer.' });
    maxLength(fields.contactPhone, 50, { message: 'Contact phone must be 50 characters or fewer.' });
    maxLength(fields.jobPosting, 2000, { message: 'Job posting URL must be 2000 characters or fewer.' });
  });
